...
<div class="col-md-4 col-sm-6 col-xs-12">
    <div class="info-box">
        ...
        <div class="info-box-content" style="padding-top:5px;">
            ...
            <small class="info-box-number"><small style="color: blue;">{{ number_format(App\RefillCustomer::with('user')->where('created_by', Auth::user()->id)->whereColumn('created_at', 'updated_at')->where('created_at', '>=', Carbon\Carbon::today())->pluck('amount_paid')->sum() + App\RefillCustomer::with('user')->where('created_by', Auth::user()->id)->where('updated_at', '>=', Carbon\Carbon::today())->pluck('second_paid')->sum()) }}</small> Total Income</small>
        </div>
    </div>
</div>
